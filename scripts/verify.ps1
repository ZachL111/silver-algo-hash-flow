...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver-algo-hash-detail.ps1

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver-algo-hash-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver-algo-hash-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver_algo_hash_flow-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
