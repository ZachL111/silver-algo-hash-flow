...
`silver-algo-hash-flow` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for hash analysis with windowed input fixtures, late-data behavior checks, and documented operating limits.
...
The score is deterministic. The current threshold is 178. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 4.
